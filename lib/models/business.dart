...
class BusinessModel {
  ...
  factory BusinessModel.fromMap(Map<String, dynamic> map) {
    ...
    final double totalInvestment =
        (map['totalInvestment'] ?? expenses).toDouble();
    final int expansionLevel = (map['expansionLevel'] ?? 0).toInt();
    final double nextExpansionCost =
        (map['nextExpansionCost'] ?? (expenses * 0.5)).toDouble();
    ...
    return BusinessModel(
      ...
      expansionLevel: expansionLevel,
      nextExpansionCost: nextExpansionCost,
      nextExpansionAvailableAt: map['nextExpansionAvailableAt'] is Timestamp
          ? map['nextExpansionAvailableAt']
          : null,
    );
  }
}
