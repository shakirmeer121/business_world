...
class BusinessService {
  ...
  Future<BusinessModel> getBusinessById(String id) async {
    ...
    if (data['ownerId'] != uid) throw Exception('Unauthorized');
  ...
  Future<void> addBusiness(BusinessModel business) async {
    ...
    final data = business.toMap()
      ...
      ..['nextExpansionAvailableAt'] =
          business.nextExpansionAvailableAt ?? FieldValue.serverTimestamp()
  ...
  Future<void> updateBusiness(String id, Map<String, dynamic> updatedFields) async {
    ...
    if (!docSnapshot.exists) throw Exception("Business not found");
  ...
  Future<void> expandBusiness(String id) async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      if (!businessSnap.exists) throw Exception('Business not found');

      final b = businessSnap.data() as Map<String, dynamic>;
      if (b['ownerId'] != uid) throw Exception('Unauthorized');

      final financeSnap = await transaction.get(financeRef);
      if (!financeSnap.exists) throw Exception('User finance doc not found');
      ...
      final double nextCost = (b['nextExpansionCost'] ??
              ((b['expenses'] ?? 0).toDouble() * 0.5))
          .toDouble();

      final Timestamp now = Timestamp.now();
      final Timestamp? availableAt =
          b['nextExpansionAvailableAt'] is Timestamp
              ? b['nextExpansionAvailableAt'] as Timestamp
              : null;

      if (availableAt != null &&
          now.millisecondsSinceEpoch < availableAt.millisecondsSinceEpoch) {
        throw Exception('Expansion not available yet');
      }

      if (cash < nextCost) throw Exception('Insufficient cash');

      // Update finance balances
      finance['cash'] = cash - nextCost;
      finance['businesses'] =
          ((finance['businesses'] ?? 0).toDouble()) + nextCost;

      // Update business
      final double currentIncomePerMinute =
          (b['incomePerMinute'] ?? 0).toDouble();
      final double newIncomePerMinute =
          currentIncomePerMinute * (1 + expansionIncomeIncreaseFactor);

      final double currentInvestment =
          (b['totalInvestment'] ?? (b['expenses'] ?? 0)).toDouble();
      ...
      final Map<String, dynamic> updates = {
        ...
        'nextExpansionAvailableAt': Timestamp.fromMillisecondsSinceEpoch(
          now.millisecondsSinceEpoch + expansionCooldown.inMilliseconds,
        ),
      };
  ...
  Future<double> sellBusiness(String id) async {
    ...
    await FirebaseFirestore.instance.runTransaction((transaction) async {
      ...
      finance['businesses'] =
          ((finance['businesses'] ?? 0).toDouble()) - totalInvestment;

      if ((finance['businesses'] as num).toDouble() < 0) {
        finance['businesses'] = 0.0;
      }
